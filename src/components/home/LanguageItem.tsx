...
function LanguageItem({language, loading = false, displayChildren = false, ...rest}: LanguageItemProps) {
  ...
  const colorId = `color-panel-${language.code}-${Math.random()}`;
  useEffect(() => {
  ...
  if (!displayChildren) {
    return (
      <Link href={`/by-language/${language.code}`}>
        <LanguagePaper {...rest} id={colorId} style={{backgroundColor: fadedColor || dominantColor}}>
          <SvgIconByName name={language.code} />
          <TextPaper elevation={0}>{language.name}</TextPaper>
        </LanguagePaper>
      </Link>
    );
  }
  ...
  return (
    <LanguagePaper {...rest} id={colorId} style={{backgroundColor: fadedColor || dominantColor}}>
      ...
      >
        {[
          ...new Set([
            ...(languagesHierarchy.list || []).filter((l) => (language.inspiring || []).includes(l.code)),
            language,
          ]),
        ].map((language) => (
